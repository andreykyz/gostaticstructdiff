...
func ComplexStructPatch(original, new ComplexStruct) ComplexStructDiff {
	...
	// Map diff with nested struct values
	diff.NestedMapArray = &struct {
		Add map[string]models.Metadata
		Modify map[string]models.MetadataDiff
		Del map[string]struct{}
	}{}
	diff.NestedMapArray.Add = make(map[string]models.Metadata)
	diff.NestedMapArray.Modify = make(map[string]models.MetadataDiff)
	diff.NestedMapArray.Del = make(map[string]struct{})
	// Added or modified keys
	for k, v := range new.NestedMapArray {
		oldV, ok := original.NestedMapArray[k]
		if !ok {
			// added key
			diff.NestedMapArray.Add[k] = v
		} else if !reflect.DeepEqual(oldV, v) {
			// modified key, compute diff
			nestedDiff := models.MetadataPatch(oldV, v)
			diff.NestedMapArray.Modify[k] = nestedDiff
		}
	}
	// Deleted keys
	for k := range original.NestedMapArray {
		if _, ok := new.NestedMapArray[k]; !ok {
			diff.NestedMapArray.Del[k] = struct{}{}
		}
	}
	if len(diff.NestedMapArray.Add) == 0 && len(diff.NestedMapArray.Modify) == 0 && len(diff.NestedMapArray.Del) == 0 {
        diff.NestedMapArray = nil
    }
	// Nested struct diff
...
func ApplyComplexStructDiff(original ComplexStruct, diff ComplexStructDiff) ComplexStruct {
	...
	if diff.NestedMapArray != nil{
		// Apply map diff
		if result.NestedMapArray == nil {
			result.NestedMapArray = make(map[string]models.Metadata)
		}
		for k, v := range diff.NestedMapArray.Add {
			// v is a models.Metadata (whole value), assign directly
			result.NestedMapArray[k] = v
		}
		for k, v := range diff.NestedMapArray.Modify {
			// v is a models.MetadataDiff, apply it to the existing value (or zero)
			existing, ok := result.NestedMapArray[k]
			if !ok {
				existing = models.Metadata{}
			}
			result.NestedMapArray[k] = models.ApplyMetadataDiff(existing, v)
		}
		for k := range diff.NestedMapArray.Del {
			delete(result.NestedMapArray, k)
		}
	}
...
// NestedArrayDiff represents the diff of a NestedArray struct.
type NestedArrayDiff struct {
...
// NestedArrayPatch computes the diff between original and new NestedArray.
func NestedArrayPatch(original, new NestedArray) NestedArrayDiff {
	var diff NestedArrayDiff
	// Slice comparison using reflect.DeepEqual
	if !reflect.DeepEqual(original.NestedStringArray, new.NestedStringArray) {
		diff.NestedStringArray = &struct { Value []string }{}
		diff.NestedStringArray.Value = new.NestedStringArray
	}
	return diff
}

// ApplyNestedArrayDiff applies a diff to an original NestedArray.
func ApplyNestedArrayDiff(original NestedArray, diff NestedArrayDiff) NestedArray {
	result := original
	if diff.NestedStringArray != nil{
		result.NestedStringArray = diff.NestedStringArray.Value
	}
	return result
}
